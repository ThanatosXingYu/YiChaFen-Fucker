...
- 双标签页：`查询` 用于批量导出结果，`锁定` 用于逐条独立会话提交结果页锁定接口。
...
- 运行缓存自动清理：任务正常完成后自动清理缓存，也保留手动清理按钮备用。
...
5. 按策略处理验证码。
6. 查询成功一条就立即写入结果文件。
7. 在“锁定”标签页中，查询成功后会进入结果页并访问页面里的“锁定”入口，输出基础查询字段和锁定访问状态。
...
1. 先选择顶部的“查询”或“锁定”标签页。
2. 在“链接解析”里输入查询主页链接或单条查询链接，点击“解析链接”。
3. 在“查询项”里选择要查询的项目，程序会自动刷新查询条件。
4. 点击“上传数据文件”，选择 `txt`、`csv`、`xls` 或 `xlsx`。
5. 如果是文本/CSV，可修改分隔符后点击“重新解析并预览”；如果是 Excel，选择工作表。
6. 在“查询条件与数据列匹配”中，把每个查询条件映射到本地列头。
7. 选择验证码策略，并按需要填写刷新请求条件。
8. 选择导出格式和保存目录，文件名会在开始处理时自动生成。
9. 点击“开始查询”或“开始锁定”。
...
cache/temp/      临时文件
cache/tools/     下载工具和后续扩展工具缓存
ycf_app/         主程序模块
```

“清理缓存”按钮只清理 `cache/cookies/`、`cache/captcha/`、`cache/temp/`、`cache/tools/`，不会删除 `output/` 里的查询结果。
